...
const client = (dir, comp, encode) => {
    ...
    filename: 'Svelte' + encode(comp) + '.js',
		format: 'iife',
    globals: (relPath) => {
      const data = fs.readFileSync(pathUtil.resolve(dir, relPath), 'utf8');
...
const loader = (src, encode) => {
  ...
  return {
    render: partial(render, [src]),
    client: partial(client, [dir, data, encode]),
    styles: partial(style, [data]),
    constructor: partial(constructor, [name]),
...
const encodeContent = (content, salt = 'salt',) => {
  const hashids = new Hashids(salt, 16, hashAlphabet);
  return hashids.encode(positiveHashCode(content));
};

// export const variableName = (content, seed=128, salt = 'salt-me') =>
//   mmh3.murmur128HexSync(salt + content, seed);
...
var lib_1 = lib.parse;
var lib_2 = lib.parseFragment;
var lib_3 = lib.serialize;
var lib_4 = lib.treeAdapters;
 ...
 */
function* depthFirst(node, getChildNodes = util.defaultChildNodes) {
    yield node;
 ...
 */
function* depthFirstReversed(node, getChildNodes = util.defaultChildNodes) {
    const childNodes = getChildNodes(node);
    if (childNodes !== undefined) {
...
function depthFirstIncludingTemplates(node) {
    return depthFirst(node, util.childNodesIncludeTemplate);
}
 ...
 */
function query(node, predicate, getChildNodes = util.defaultChildNodes) {
 ...
 * predicate.
 */
function* queryAll(node, predicate, getChildNodes = util.defaultChildNodes) {
 ...
 */
function nodeWalk(node, predicate, getChildNodes = util.defaultChildNodes) {
 ...
 * returned.
 */
function nodeWalkAll(node, predicate, matches, getChildNodes = util.defaultChildNodes) {
 ...
 */
function queryAll(node, predicate, matches, getChildNodes = util.defaultChildNodes) {
    const elementPredicate = predicates.predicates.AND(predicates.isElement, predicate);
...
exports.childNodesIncludeTemplate = function childNodesIncludeTemplate(node) {
    if (node.nodeName === 'template') {
        return lib.treeAdapters.default.getTemplateContent(node).childNodes;
    }
    return node.childNodes;
};
...
unwrapExports(util);
var util_1 = util.getTextContent;
var util_2 = util.getAttribute;
var util_3 = util.getAttributeIndex;
var util_4 = util.hasAttribute;
var util_5 = util.setAttribute;
var util_6 = util.removeAttribute;
var util_7 = util.normalize;
var util_8 = util.setTextContent;
var util_9 = util.defaultChildNodes;
var util_10 = util.childNodesIncludeTemplate;
...
function hasTextValue(value) {
    return function (node) {
        return util.getTextContent(node) === value;
    };
}
function OR( /* ...rules */) {
...
function hasAttr(attr) {
    return function (node) {
        return util.getAttributeIndex(node, attr) > -1;
    };
}
function hasAttrValue(attr, value) {
    return function (node) {
        return util.getAttribute(node, attr) === value;
    };
}
...
function hasSpaceSeparatedAttrValue(name, value) {
    return function (element) {
        const attributeValue = util.getAttribute(element, name);
...
unwrapExports(lib$1);
var lib_1$1 = lib$1.query;
var lib_2$1 = lib$1.queryAll;
var lib_3$1 = lib$1.replace;
var lib_4$1 = lib$1.append;
...
const parse = (html) => lib.parse(html, {scriptingEnabled:false});

const serialize = (dom) => lib.serialize(dom);

const appendToHead = (dom, headElements = []) => {
  const headSet = new Set(headElements);
  const head = lib_1$1(dom, (el) => el.tagName === 'head');
  headSet.forEach(elementHtml => 
    lib_4$1(head, lib.parseFragment(elementHtml, {scriptingEnabled:false})));
};
...
const domRefs = (dom, resolvers = {Default: (attr) => ''}) => {
  ...
  lib_2$1(body, (el) => isCustomElement(el)).forEach((el, i) => {
    ...
    if (name in resolvers) {
      const newEl = lib.parseFragment(`<div id="${id}" ${serializeAttribs(appliedAttribs)}>`+resolvers[name](attribs)+'</div>', {scriptingEnabled:false});
      lib_3$1(el, newEl);
    } else {
      const newEl = lib.parseFragment(`<div id="${id}" ${serializeAttribs(appliedAttribs)}>`+resolvers.Default(attribs)+'</div>', {scriptingEnabled:false});
      lib_3$1(el, newEl);
    }
  });
  return refs;
...
const allLoaders = (componentPaths, partialLoader, encode) => {
...
const htmlResolversFromLoaders = (loaders) => {
...
const compile = (html, opts = defaultOpts, loader$1 = loader) => {
  const encode = partialRight(encodeContent, [opts.salt]);
  const dom = parse(html);
  const loaders = allLoaders(opts.components, loader$1, encode);
  const refs = domRefs(dom, htmlResolversFromLoaders(loaders));
  
  const js = [...refs.entries()].map(([name, ...instances]) => {
    const constructors = instances.flat().map(instance => loaders.get(name).constructor(instance.id, instance.attr));
  ...
  const css = [...refs.keys()].map((name) => {
    ...
    result.forEach(content => {
      appendToHead(dom, [stylesheet(encode(content))]);
    });
    return result;
  }).flat(); 
  return {
