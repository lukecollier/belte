...
const client = (src, encode) => {
  const comp = fs.readFileSync(src, 'utf8');
...
const constructor = (name, componentName, id, attr) => 
...
const loader = (src, encode) => {
  ...
  return {
    ...
    client: partial(client, [src, encode]),
    styles: partial(style, [src]),
    constructor: partial(constructor, [name, componentName]),
